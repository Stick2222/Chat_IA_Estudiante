...
`;
  ...
  private isWorstGradeQuery(message: string): boolean {
    const worstGradeKeywords = [
      'peor nota', 'menor calificacion', 'nota menor',  'nota mas baja', 'peor calificacion',
  ...
  private generateWorstGradeResponse(inscripciones: any[]): string {
    ...
    if (calificacion < 70) {
      response = this.formatResponse(`⚠️ Materia que Requiere Atención\n\n${materia?.nombre || 'Materia no disponible'}\n\nCalificación actual: ${calificacion}/100\nCarrera: ${peorInscripcion.carrera?.nombre || 'No disponible'}\nAula: ${peorInscripcion.paralelo?.aula || 'No disponible'}\n\nEsta materia necesita atención prioritaria. Te recomiendo:\n\n💡 Estrategias Inmediatas:\n• Solicita tutoría con tu profesor\n• Revisa los temas que no comprendiste\n• Forma un grupo de estudio\n• Dedica tiempo adicional diario a esta materia\n• Consulta recursos adicionales en línea\n\n¿Te gustaría que genere un roadmap detallado para mejorar en ${materia?.nombre || 'esta materia'}?`);
    } else {
      response = this.formatResponse(`📊 Materia con Menor Calificación\n\n${materia?.nombre || 'Materia no disponible'}\n\nCalificación: ${calificacion}/100\n\nAunque esta es tu calificación más baja, está dentro del rango aprobatorio. Aún así, hay oportunidad de mejorar para alcanzar la excelencia.`);
    }
